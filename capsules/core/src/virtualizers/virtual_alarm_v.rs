...
use kernel::collections::list_v::{ListIteratorV, ListLinkV, ListNodeV, ListV, GhostState};
...
pub tracked struct VirtualMuxAlarmState<'a, A: Alarm<'a>> {
    pub tracked dt_reference_pt: PointsTo<TickDtReference<A::Ticks>>,
    pub tracked armed_pt: PointsTo<bool>,
    pub tracked next_pt: PointsTo<VirtualMuxAlarm<'a, A>>,
}
...
impl<'a, A: Alarm<'a>> VirtualMuxAlarm<'a, A> {
    pub closed spec fn valid_state(&self, state: &Tracked<VirtualMuxAlarmState<'a, A>>) -> bool {
        ...
        &&& self.next.0.id() == state@.next_pt.id()
        &&& state@.dt_reference_pt.is_init()
        &&& state@.armed_pt.is_init()
        &&& state@.next_pt.is_init()
    }
    ...
    pub fn new(mux_alarm: &'a MuxAlarm<'a, A>) -> (res: (
        ...
        ensures
            ...
            res.0.next.0.id() == res.1@.next_pt.id(),
            res.1@.dt_reference_pt.is_init(),
            res.1@.armed_pt.is_init(),
            res.1@.next_pt.is_init(),
            res.1@.armed_pt.value() == false,
    {
        ...
        let (next, Tracked(next_pt)) = ListLinkV::empty();
        (
            VirtualMuxAlarm {
                mux: mux_alarm,
                dt_reference,
            ...
            Tracked(VirtualMuxAlarmState { dt_reference_pt, armed_pt, next_pt }),
        )
    }
...
impl<'a, A: Alarm<'a>> Alarm<'a> for VirtualMuxAlarm<'a, A> {
    fn disarm(&self, state: &mut Tracked<VirtualMuxAlarmState<'a, A>>) -> Result<(), ErrorCode>
    ensures
    ...
    fn is_armed(&self, state: &Tracked<VirtualMuxAlarmState<'a, A>>) -> (res: bool)
    ensures
    self.valid_state(state),
...
impl<'a, A: Alarm<'a>> time::AlarmClient for MuxAlarm<'a, A> {
    ...
    fn alarm(&self, state: &Tracked<MuxAlarmState<'a, A>>) {
        ...
        let mut iterator = ListIteratorV::new(self.virtual_alarms.head(), &state.virtual_alarms_state);
